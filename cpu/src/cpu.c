...
int ejecutar_ciclo_de_instruccion(t_contexto_ejecucion* ctx, int socketConexionMemory, t_log* loggerCpu) {
    ...
    char *instruccionFormatoString = buffer_read_string(buffer, longitudInstruccion);

    log_info(loggerCpu, "CPU: Se obtuvo el string instruccion");

    Instruccion_codigo tipoInstruccion = obtener_instruccion_registro_valor(instruccionFormatoString); //Esto me modifica mi instruccionFormatoString
    ...
    decode(tipoInstruccion, instruccionFormatoString);
}
...
int obtener_instruccion_registro_valor(char *string) {
    ...
    while ((string[i] != ' ') && (string[i] == '\0')) {
        bufferTemporal[i] = string[i];
        if (isdigit(bufferTemporal[i])) {
            //Entonces es un numero
            int numero = bufferTemporal;
            return numero;
        }
        i++;
    }

    string = string[i + 1]; //Para luego leer desde ahi

    for (int j = 0; instrucciones[j].nombreDeLaInstruccion != NULL; j++)
    {
        if(strcmp(bufferTemporal, instrucciones[j].nombreDeLaInstruccion) == 0)
        {
            return instrucciones[j].instruccion_codigo;
        }
    }
    ...
    for (int k = 0; registros[k].nombreDelRegistro != NULL; k++)
    {
        ...
        {
            return registros[k].codigo_registros_cpu;
        }
    }
